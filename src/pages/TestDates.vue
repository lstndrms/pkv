      ...
      if (u_td.length !== 0) {
        ...
        u_td.forEach((val) => {
                ...
                if (val.has_results) {
                    ...
                    this.grades.push(
                        {'column1': val.date, 'column2': 'Русский язык', 'column3': (val.russian_language_grade.is_valid) ? val.russian_language_grade.val : '-', 'column4': (uData.education_year === 8 ? '7' : '10')},
                        {'column1': val.date, 'column2': 'Математика', 'column3': (val.math_grade.is_valid) ? val.math_grade.val : '-', 'column4': '10'},
                        {'column1': val.date, 'column2': 'Иностранный язык', 'column3': (val.foreign_language_grade.is_valid) ? val.foreign_language_grade.val : '-', 'column4': (uData.education_year === 10 ? '10' : (uData.education_year === 8 ? '23' : '30'))},
                        {'column1': val.date, 'column2': uData.first_profile_subject.name, 'column3': (val.first_profile_grade.is_valid) ? val.first_profile_grade.val : '-', 'column4': (uData.education_year === 10 ? '20' : '10')},
                        {'column1': val.date, 'column2': uData.second_profile_subject.name, 'column3': (val.second_profile_grade.is_valid) ? val.second_profile_grade.val : '-', 'column4': (uData.education_year === 10 ? '20' : '10')},
                    )
                    if (sumFirstProfile) {
